...
public interface StockPriceRepository extends JpaRepository<StockPriceDetails, Long> {
	...
	@Query("select sp FROM StockPriceDetails sp WHERE sp.companyCode=?1 AND sp.stockPriceDate BETWEEN ?2 AND ?3")
	public List<StockPriceDetails> findStockIndexByCompanyCode(Long companyCode, LocalDate from, LocalDate to);
}